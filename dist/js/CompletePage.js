...
document.addEventListener('DOMContentLoaded', function () {
  ...
  // Mapa de estado del contenido y los iconos correspondientes
  ...
  // Función para obtener el estado del pago desde Stripe
  ...
  // Función para actualizar el contenido de estado en el DOM
  function updateStatusContent() {
    ...
    const detailsTable = statusContainer.querySelector('#details-table');
    const intentIdElement = document.getElementById('intent-id');
    const intentStatusElement = document.getElementById('intent-status');
    const viewDetailsLink = document.getElementById('view-details');

    statusIcon.style.backgroundColor = statusContent.iconColor;
    statusIcon.innerHTML = '';
    statusIcon.appendChild(statusContent.icon);

    statusText.textContent = statusContent.text;

    if (intentId) {
      detailsTable.style.display = 'block';
      intentIdElement.textContent = intentId;
      intentStatusElement.textContent = status;
  ...
  paymentStatusContainer.id = 'payment-status';
  paymentStatusContainer.innerHTML = `
    <div id="status-icon"></div>
    <h2 id="status-text"></h2>
    <div id="details-table" style="display: none;">
      <table>
        <tbody>
          <tr>
    ...
    <a id="view-details" href="#" rel="noopener noreferrer" target="_blank">View details</a>
    <a id="retry-button" href="./checkout">Test another</a>
  `;

  // Añadir el contenedor al DOM
  document.body.appendChild(paymentStatusContainer);

  // Llamar a la función para recuperar el estado del pago
